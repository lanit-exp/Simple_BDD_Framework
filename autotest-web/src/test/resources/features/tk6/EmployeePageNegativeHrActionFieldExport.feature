...
Функция: Проверка поведения системы при невалидном вводе в поле поиска. Страница "Сотрудники" по ролью HR
  ...
  после нажатия на кнопку "Сотрудники", проверка поведения системы при экспорте списка сотрудников через опцию выпадающего списка при не выбранном формате экспортируемого файла
    ...
    Примеры:
      |url                          |login |password    |button1   |element         |dropdown     |button2  |textdropdown                       |field                      |text                              |
      |http://178.154.246.238:58082/|hr    |hrmhrm12345 |Сотрудники|Таблица чек-бокс|Действие меню|Выполнить|Экспортировать выбранные Сотрудники|Предупреждение в заголовке |Необходимо выбрать формат экспорта|
