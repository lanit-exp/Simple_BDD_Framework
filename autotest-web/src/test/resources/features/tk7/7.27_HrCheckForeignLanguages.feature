#language:ru
@djangoHr
Функционал: Проверка корректности работы блока Иностранные языки под ролью HR
  ...
  Сценарий: Нажать кнопку 'Иностранные языки', в текущем блоке нажать
  на ссылку текстом 'Добавить еще один Иностранный язык', проверить, что
  в текущем блоке поле 'Иностранный язык' не заполнено, затем нажать на ссылку
  текстом 'Скрыть', проверить, что поле 'Иностранный язык' отсутствует

    Дано инициализация страницы "DjangoEmployeeChange"
    Если проскроллить страницу до элемента "Иностранные языки"
    Тогда нажать на "Иностранные языки"
    Если на странице имеется элемент "Добавить еще один Иностранный язык"
    Тогда нажать на "Добавить еще один Иностранный язык"
    Если чекбокс "Иностранный язык" отображается и не выбран
    Тогда нажать на "Скрыть"
    Если на странице отсутствует элемент "Иностранный язык"