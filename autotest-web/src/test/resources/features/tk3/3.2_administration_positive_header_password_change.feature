...
Функционал: Проверка работоспособности кнопки "Изменить пароль"

  Сценарий: Авторизоваться от лица админ, открыть страницу Django Administration, нажать на кнопку "Изменить пароль"
    ...
    Если нажать на "Изменить пароль"
    То инициализация страницы "DjangoPasswordChange"
    И на странице имеется элемент "старый пароль"
